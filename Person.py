...
class Person(PersonBase):
    ...
    def __init__(
        ...
        self.attr["quanta_emission_rate"] = (
            quanta_emission_rate if quanta_emission_rate else 147
        )
    ...
    def expose_person_to_quanta(self, quanta_concentration: float) -> None:
        ...
        if random.random() < self.infection_risk_instant(quanta_concentration):
            if self.state["infection_status"] == "susceptible":
                self.log_infection()
                self._sim_env.dc.counter_increment("Infections")
    ...
    def infection_risk(self):
        ...
        return 1 - math.exp(
            -1
            * self.attr["inhalation_rate"]
            * self._sim_env.time_interval
            * self.attr["cumulative_exposure"]
        )
    ...
    def log_infection_risk(self):
        ...
        self._sim_env.dc.log_reporting(
            "Infection risk",
            {"Person": self.id, "Infection risk": self.infection_risk()},
        )

    def log_infection(self):
        """Log visitor activity within the process visitor process"""
        self._sim_env.dc.log_reporting("Infections", {"Person": self.id})
