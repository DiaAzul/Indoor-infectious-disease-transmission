...
class PersonBase(AttrActions):
    ...
    def run(self) -> Generator[simpy.events.Event, Any, Any]:
        ...
        function_dict = {
            ...
            "run_a": self._run_a,
            "run_b": self._run_b,
            "b_to_a": self._transfer_b_to_a,
        }

        machine_state = "init"
        received_message = "initialise_a"
        activity_a = self._get_next_activity(self.starting_node_id)
        activity_b = None
        ...
        while not finished:
            ...
            actions = PersonBase._state_diagram.get(machine_state, "NoState").get(
                status.received_message, "NoMessage"
            )
            if actions == "NoState":
                raise ValueError(
                    f"Person state error:s->{machine_state}:m->{received_message}"
                )
            if actions == "NoMessage":
                raise ValueError(
                    f"Person received message error:s->{machine_state}:m->{received_message}"
                )
    ...
    def _nop(self, status: Status) -> Status:
        """Function which does nothing
        """
        return status

    def _run_a(self, status: Status) -> Status:
        if status.activity_a is not None:
            self._run_activity(status.activity_a)
        return status

    def _run_b(self, status: Status) -> Status:
        if status.activity_b is not None:
            self._run_activity(status.activity_b)
        return status

    def _transfer_b_to_a(self, status: Status) -> Status:
    ...
    def _run_activity(self, activity):
        activity_class = activity.activity_class(self._sim_env, **activity.kwargs)
        self._sim_env.env.process(activity_class.run())

    # TODO: Still need to implement decision making logic
    def _get_next_node(self, status: Status) -> Status:
        if status.activity_a.graph_ref[1] == "end":
            ...
            decision = self._sim_env.routing.get_decision_from_activity_ref(
                status.activity_a.kwargs["graph_activity_ref"]
            )
            status.activity_b = self._get_next_activity(decision.id)
            status.received_message = "initialise_b"

        return status

    def _get_next_activity(self, decision_id: str) -> Activity:
        activity_list = self._sim_env.routing.get_activities_from_decision_id(
            decision_id
        )
        ...
        activity.kwargs["message_to_activity"] = simpy.Store(self._sim_env.env)
        activity.kwargs["message_to_person"] = simpy.Store(self._sim_env.env)
