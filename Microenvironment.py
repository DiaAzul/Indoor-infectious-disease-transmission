...
class Microenvironment(ResourceBase):
    ...
    def __init__(
        ...
        self.attr["time_interval"] = self._sim_env.time_interval
        ...
        self.microenvironment = simpy.Resource(self._sim_env.env, self.attr["capacity"])
    ...
    def run(self):
        ...
        while True:
            ...
            self.attr["quanta_in_microenvironment"] *= math.exp(
                -1 * self.attr["air_exchange_rate"] * self.attr["time_interval"]
            )

            yield self._sim_env.env.timeout(1)
    ...
    def initialise_periodic_reporting(self):
        ...
        self._sim_env.dc.create_period_reporting(data_set_name, callback, periods)

    def periodic_reporting_callback(self):
        """ Callback to collect data for periodic reporting """
        return {
            f"Quanta concentration {self.attr['environment_name']}": self.get_quanta_concentration()
        }
